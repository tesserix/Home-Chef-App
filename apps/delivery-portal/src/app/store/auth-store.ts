...
/**
 * Delivery portal auth store. All delivery-portal users are drivers under the
 * business GIP pool — the previous staff/driver dual-mode split has been
 * collapsed into a single flow. Staff identity (fleet managers etc.) is
 * derived from the staff-profile API endpoint, not from auth mode.
 */
interface AuthState {
