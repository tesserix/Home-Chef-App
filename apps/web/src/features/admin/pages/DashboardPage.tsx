...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Lead block — Revenue today (dominant) + pending verifications CTA */}
      ...
      >
        ...
        <StatRow
          ...
          subtitle={`${stats?.totalOrders?.toLocaleString() ?? '0'} orders all-time`}
        />
      </section>

      {/* Work area — asymmetric 2:1, recent activity dominant */}
      <div className="grid gap-8 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
        {/* Recent Activity */}
        <section aria-label="Recent activity" className="space-y-3">
          <div className="flex items-baseline justify-between">
            <h2 className="text-lg font-semibold text-foreground">Recent activity</h2>
            <Link to="/admin/activities" className="text-sm font-medium text-herb hover:underline">
              View all
            </Link>
          </div>
        ...
        <aside aria-label="Shortcuts" className="space-y-3">
          ...
          <nav className="divide-y divide-mist rounded-lg border border-mist bg-bone">
            <QuickAction to="/admin/users" title="Manage users" subtitle="View all users" />
            <QuickAction to="/admin/chefs" title="Chef verification" subtitle="Review applications" />
            <QuickAction to="/admin/orders" title="Order management" subtitle="Track all orders" />
            <QuickAction to="/admin/analytics" title="Analytics" subtitle="View reports" />
          </nav>
        </aside>
      </div>
    </div>
