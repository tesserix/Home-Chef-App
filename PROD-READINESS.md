...
### Mobile
- [~] **TestFlight pipeline** — pipeline **DONE 2026-06-16**: org Apple Developer account (Team `2CRHRRYBPL`), ASC app records (vendor `6780689641` / customer `6780689976`), dist cert + provisioning profiles, ASC API key on EAS → both apps built (`.ipa`) + submitted to TestFlight; Android both published to Play internal + auto-submit verified; **full release pipeline now zero-touch both stores** (`eas workflow:run release.yml`). Bundle IDs migrated to `com.tesserix.homechef.*`. **✅ launch crash + Google Sign-In + Apple Sign-In ALL RESOLVED on-device (2026-06-16):** verified on a local signed device build (iPhone 17 Pro), both apps — launch, Google, and Apple sign-in all work. Arch-mismatch theory was wrong (RN 0.83 is new-arch-only). (1) Launch crash resolved in current code. (2) Google Sign-In was a stale iOS-client-ID mismatch (`.env.local` vs `GoogleService-Info.plist`) — fix commit dda168e: aligned client IDs, added google-signin plugin `iosUrlScheme`, added Google client IDs + `GIP_API_KEY` to `eas.json` production env. (3) Apple Sign-In was `auth/operation-not-allowed` — fixed by enabling Apple provider in both GIP tenants (server-side, no app change). **EAS production build #3 submitted to TestFlight** (vendor `fe77a977` + customer `3c45c479`). Remaining: confirm #3 from TestFlight; ship Android via `release.yml`. See `project_ios_launch_crash_debug` + `project_apple_signin_gip_not_enabled` memories.
