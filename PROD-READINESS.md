...
### Mobile
- [~] **TestFlight pipeline** — pipeline **DONE 2026-06-16**: org Apple Developer account (Team `2CRHRRYBPL`), ASC app records (vendor `6780689641` / customer `6780689976`), dist cert + provisioning profiles, ASC API key on EAS → both apps built (`.ipa`) + submitted to TestFlight; Android both published to Play internal + auto-submit verified; **full release pipeline now zero-touch both stores** (`eas workflow:run release.yml`). Bundle IDs migrated to `com.tesserix.homechef.*`. **🔴 BLOCKER: both apps CRASH ON LAUNCH on the TestFlight builds** — native crashes (not JS): vendor SIGSEGV via NSInvocation, customer uncaught ObjC exception. Needs dSYM symbolication to pinpoint (suspects: Firebase native init / expo-updates). See `project_ios_launch_crash_debug` memory + `.ips` files on Desktop. **Must fix before any on-device testing.**
