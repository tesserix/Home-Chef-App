...
Last activity: 2026-06-17 - Completed quick task 260617-lxn: Error-code UX (vendor login auth errors → resolveAuthErrorMessage); branch fix/auth-error-code-ux, PR pending
...
| 260617-lxn | Error-code UX (owner priority) — route auth errors through `resolveAuthErrorMessage` so users never see raw codes (`auto_login_502`, Firebase `auth/*`). **Scope finding: only mobile-vendor `(auth)/login.tsx` leaked** (4 `Alert.alert(..., err.message)` blocks); customer/delivery login + all register/forgot-password already delegate to shared screens that map+banner. Fixed vendor login (−12/+5). Branch `fix/auth-error-code-ux` → PR (NOT main). Follow-ups: align vendor to inline-banner pattern; app-wide non-auth `err.message` audit needs a generic resolver | 2026-06-17 | d987f8d | [260617-lxn-error-code-ux-replace-raw-err-message-wi](./quick/260617-lxn-error-code-ux-replace-raw-err-message-wi/) |
