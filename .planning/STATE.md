...
- **✅ RESOLVED on-device (2026-06-16): iOS launch crash + Google Sign-In crash.** Verified on a local signed device build (iPhone 17 Pro): both apps launch + Google Sign-In work. The earlier arch-mismatch theory was WRONG (RN 0.83 is new-arch-only; `newArchEnabled` is a no-op). Two bugs: (1) **launch crash** resolved in current code (the crashed TestFlight builds were an older commit; sim couldn't reproduce it). (2) **Google Sign-In** `NSInvalidArgumentException: missing URL scheme …<client>` — stale pre-migration iOS OAuth client IDs in `.env.local` didn't match `GoogleService-Info.plist` / the URL scheme. FIX (commit dda168e): aligned `.env.local` iosClientId to GoogleService-Info, added the `@react-native-google-signin/google-signin` plugin `iosUrlScheme` to both app.json, and added the missing Google client IDs + `GIP_API_KEY` to `eas.json` **production** env. **EAS production build #3 (vendor `fe77a977` + customer `3c45c479`) building + auto-submitting to TestFlight for final on-device confirmation.** Then run Android via `release.yml`. Detail in `project_ios_launch_crash_debug` memory.
