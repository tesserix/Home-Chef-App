...
| 260705-667 | GH #390 — **chef paid NET, transfer==statement** (was: Route transfer paid GROSS while statements claimed net-of-commission/TDS). Reconciled ONE formula: `gross=food(−discount)+Tax(food GST)+tip` (tax IN, delivery OUT), `net=gross−6%comm−1%TDS`; chef Route transfer now = `ComputeOrderEarnings(order).NetPayout` via unified `chefNetPayout` (3 sites: FSSAI audit/Route split/Stripe). **Frozen `Order.CommissionRate` col** (rate resolved once at checkout, read by verify+statement+tds+earnings — no drift if admin retunes; plan-check catch B2). Conservation excludes GST-on-commission (B1). Owner decision: chef gets food-GST. TDS/statement/breakdown all fixed to match. TDD (build/vet/gofmt/test green incl. #387/388/456/457/459). Behind ORDER_PAYOUT_AUTO_RELEASE_ENABLED (off). Branch `feat/390-net-chef-payout` → PR pending. Follow-ups: meal-plan-day may still pay gross; historical statements+TDS Form-16A on old basis (not rewritten); TCS §52 CA-pending | 2026-07-05 | e6c35d8 | [260705-667-390-net-chef-payout-transfer-statement-n](./quick/260705-667-390-net-chef-payout-transfer-statement-n/) |
